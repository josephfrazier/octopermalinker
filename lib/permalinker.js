  ...
  selectAll(document, '.comment').forEach((commentEl) => {
    ...
    selectAll(commentEl, '.comment-body a').forEach((anchor) => {
      ...
      gh.getRepo(user, repo).listCommits({branch, until: datetime}).then((response) => {
        ...
        permaAnchor.innerText = permalink;

        anchor.parentNode.insertBefore(document.createTextNode(` ${humanTime}) `), anchor.nextSibling);
        anchor.parentNode.insertBefore(permaAnchor, anchor.nextSibling);
