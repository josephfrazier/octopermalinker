...
function getCommentsAndDatetimes(document) {
  return selectAll(document, '.comment').map(commentEl => ({
    commentEl,
    datetimeEl: commentEl.querySelector('relative-time'),
  })).filter(({ datetimeEl }) => datetimeEl);
}
...
export default (githubOptions = {}, document = global.document) => {
  ...
  return Promise.all(concatMap(comments, ({ commentEl, datetimeEl }) => {
    ...
    return selectAll(commentEl, '.comment-body a').map((anchor) => {
      ...
      return mostRecentCommit({ gh, user, repo, branch, until: datetime }).then((sha) => {
