...
export default (githubOptions = {}, document = global.document) => {
  ...
  if (document.location.host === 'gist.github.com') {
    commentsPromise = getGistFilesAndDatetimes({ gh, document });
  } else {
    commentsPromise = getCommentsAndDatetimes(document);
  }

  return Promise.resolve(commentsPromise).then(comments => Promise.all(concatMap(comments, ({ commentEl, datetime, humanTime }) =>
...
function getBranchLinks(commentEl) {
  return selectAll(commentEl, 'a').map(anchor => ({
    anchor,
...
function insertPermalink({ document, anchor, branch, sha, humanTime }) {
  ...
  insertAfter(domify(`&nbsp;(<a href="${permalink}" title="${permalink}">${humanTime}</a>) `, document), anchor);
}
