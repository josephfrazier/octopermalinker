...
export default (githubOptions = {}, document = global.document) => {
  ...
  setupPasteListeners({ gh, document });
  ...
  if (document.location.host === 'gist.github.com') {
    ...
    rootsPromise = getMarkdownBodiesAndDatetimes(document).concat(getCommentsAndDatetimes(document));
  }
...
function setupPasteListeners({ gh, document }) {
  const inputs = selectAll(document, 'input');
  const textareas = selectAll(document, 'textarea');

  inputs.concat(textareas).forEach((field) => {
