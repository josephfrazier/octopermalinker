...
function setupPasteListeners ({ githubApi, document }) {
  ...
  async function pasteListener (event) {
    ...
    // Don't paste-protect urls to a repo's homepage.
    // They're likely intentional.
    if (parse(href).path.slice(1).split('/').length === 2) {
      return href
    }
...
function getBranchLinks (rootEl) {
  return selectAll(rootEl, 'a')
    .filter(anchor => anchor.getAttribute('data-octopermalink') == null)
...
function insertPermalink ({ document, anchor, permalink, datetime }) {
  ...
  const imageUrl = global.chrome ? global.chrome.extension.getURL('link-icon.png') : '' // tests don't have global.chrome
  ...
  const permaAnchor = `<a data-octopermalink href="${permalink}" title="${permalink}">${image} ${relativeTime}</a>`
