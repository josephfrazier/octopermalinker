...
function setupPasteListeners({ document }) {
...
async function getPermalink({ href, datetime }) {
  const branch = userRepoBranch(href).branch;
  if (!branch || isCommitHash(branch)) {
    throw Error('href does not have a branch, or is to a commit hash');
  }
  const datetimeDigits = datetime.split('.')[0].replace(/[^\d]/g, '');
  return `https://timetravel.mementoweb.org/memento/${datetimeDigits}/${href}`;
}
